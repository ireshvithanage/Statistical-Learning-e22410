...
class DataInspector:
    ...
    def test_constant_mean(self, columns: Optional[Sequence[str]] = None, chunks: int = 10) -> Any:
        """
        Assesses homogeneity of first-order moments across sequential data segments using MANOVA with Wilks’ Lambda.

        The computation is numerically stabilized using log-determinant tracking and shrinkage-based regularization.

        Parameters:
        - columns: Sequence of column names (strings). If None, all numeric columns are automatically selected.
        - chunks: Integer specifying how many consecutive, non-overlapping segments the dataset should be divided into.
        """
        if self.df is None: 
